...
namespace Chess
{
    public class Movement {
            ...
            for (int move=0;move<availableMoves.Length; move++){
                ...
                if (availableMoves[move]?.x == clickedTile.location.x && availableMoves[move]?.y == clickedTile.location.y){
                    ...
                    if (!PieceSelection.waiting){
                        ...
                        if (KingAttacked()){
                            Board.Window.GameState.Text = "Шах";
        ...
        public bool CheckMate(){
            ...
            Board.Window.GameState.Text = "Мат";
            Board.Window.GameState.ForeColor = System.Drawing.Color.Firebrick;
        ...
        public bool IsKingSafe(){
            ...
            for (int i = 0; i < availableMoves.Length; i++)
                if (availableMoves[i].HasValue)
                    return true;//решение, предотвращающее мат
            return false;
        }
        ...
        PieceMove?[] Moves() {
            ...
            switch (pieceTile.piece.piecekind) {
                ...
                case PieceKind.King:
                    ...
                    if (pieceTile.piece.firstMove && Board.GetPiece(pieceTile.GetY, pieceTile.GetX + 3).firstMove) //правая рокировка
        ...
        void LineMove(PieceMove?[] moves,bool straightLine){ //для слона, ферьзя и ладьи
        ...
        PieceMove? destination(int y, int x) { //для коня и короля
